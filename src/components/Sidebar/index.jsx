...
export default function index(){
    ...
    return(
     <div className='w-[50px] md:w-[230px] h-[auto] border border-2 shadow-md py-4 rounded-tr-[50px] rounded-br-[50px] relative'>
      <div className='flex gap-4 mb-[30px] mt-[20px]'>
         <img src={tv} alt="tv" className='w-[30px] h-[30px]' />
         <h4 className='font-bold hidden sm:flex'>Movie Box</h4>
      </div>
        {data.map((item) => (
            <div className='flex gap-4 py-[15px] px-[10px] card-hover'>
               <p className='text-[#30302f] '>{item.icon}</p>
               <p className='font-semibold text-[#30302f] text-2xlg hidden sm:flex'>{item.name}</p>
            </div>
        ))}
        <div className='border border-solid border-red-300 py-2 rounded-lg px-2 mt-[30px] hidden sm:block mx-[20px]'>
